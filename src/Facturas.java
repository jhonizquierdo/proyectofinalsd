...
//        return resultado;
    ...
    public boolean insertarFactura( int id_cliente, int id_producto, int id_vendedor, String fecha,int id_vehiculo) {
      boolean resultado = false;
        try {
            String sql = "insert into factura  (id_cliente,id_producto,id_vendedor,fecha,id_vehiculo) values('" + id_cliente + "','" + id_producto+ "','" + id_vendedor + "','" + fecha + "','" + id_vehiculo + "')";
            objCone.conectar();
            Statement st = objCone.con.createStatement();
            int valor = st.executeUpdate(sql);
            if (valor > 0) {
    ...
    public boolean actualizarFactura(int id_factura, int id_cliente, int id_producto, int id_vendedor, String fecha, int id_vehiculo) {
        boolean resultado = false;
        try {
            String sql = "update factura set id_factura='"+id_factura+"',id_cliente '" + id_cliente + "', id_producto = '" + id_producto + "',id_vendedor='" + id_vendedor + "',fecha='" + fecha + "','"+ id_vehiculo +"' where id_factura = '" + id_factura + "' ";
            objCone.conectar();
            Statement st = objCone.con.createStatement();
            int valor = st.executeUpdate(sql);
            if (valor > 0) {
